 ...
 * @api public
 */
module.exports = function ipfilter(defaultIPs,opts) {
  ...
  var matchClientIp = function matchClientIp(ip, ips) {
    var mode = settings.mode.toLowerCase();
    var result = _.invoke(ips, testIp, ip, mode, ips);
  ...
  var testIp = function testIp(ip, mode, ips) {
    var constraint = this;
    ...
    if ((typeof constraint === 'undefined' ? 'undefined' : _typeof(constraint)) === 'object') {
      return testRange(ip, constraint, mode, ips);
    }
  };
  ...
  var testRange = function testRange(ip, constraint, mode, ips) {
  ...
  return function (req, res, next) {
    req.allowedIps = req.allowedIps  ? req.allowedIps.concat(defaultIPs) : false
    ...
    // this middleware
    if (!req.allowedIps || !req.allowedIps.length) {
      return next();
    }

    if (matchClientIp(ip, req.allowedIps)) {
      // Grant access
      if (settings.log) {
